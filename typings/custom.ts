...
interface genericBullet extends genericConfig{
  angle?:number;
  speed?:number; 
}
...
interface gameLevel{
  level:{
      level:number
      velocityPlayerBullet:number;//200       +=-50
      velocityEnemyBullet:number; //200       +=50    
      velocityEnemy:number; //30 (incy)       +=10
      timeEnemyBullet:number; //1000          -=100
      score:number; //10                       +=5
      life:number
  }
}
